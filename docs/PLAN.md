...
| 18 | Per-entity 2FA / "Ask for PIN" (**Cloud parity**) | Core GA hard-codes `should_2fa` to `True`; the built-in "Ask for PIN" toggle is cloud-only (keyed to `cloud.google_assistant`, backed by cloud REST). Replicated: patch `should_2fa` to read the per-entity `disable_2fa` registry option under our `ASSISTANT_ID`; add `get_entity`/`update_entity` WS commands (mirror `cloud/google_assistant/entities/*`); and patch the `entity-voice-settings` element to inject an identical `ha-checkbox` (same `ui.dialogs.voice-settings.ask_pin` key) into our row for security devices. |
