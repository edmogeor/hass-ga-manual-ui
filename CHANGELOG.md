...
- **Adding entities from the Expose page no longer crashes.** The lazy-loaded
  dialog now receives the Google Assistant (Manual) display-name entry before
  it renders.
...
- **Optional Home Assistant Cloud hiding.** Setup and the Configure action can
