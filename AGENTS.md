...
All source files live at repo root. The `build/` directory is generated by `npm run build:dist`.
...
├── frontend.ts                # TypeScript source for the frontend companion (~1370 lines)
...
├── build/                     # Generated distribution directory
│   └── custom_components/
│       └── google_assistant_manual/   # Ready-to-copy into HA
├── docs/
...
`.gitignore` excludes: `references/`, `__pycache__/`, `.pyrefly/`, `node_modules/`, `build/`
